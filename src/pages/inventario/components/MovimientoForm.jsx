...
export default function MovimientoForm({ isOpen, onClose, onSuccess, tipo, productoId: initialProductoId }) {
  const { user } = useAuth()
  ...
  async function fetchInitialData() {
    ...
    try {
      ...
      // If no initial product, fetch all products for select
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(qty) || qty <= 0) return alertError('Validación', 'La cantidad debe ser mayor a 0')

    if (tipo === 'SALIDA' && qty > stockActual) {
      return alertError('Stock insuficiente', `Solo hay ${stockActual} unidades disponibles.`)
    }

    if (!origen) return alertError('Validación', 'Selecciona el origen del movimiento')
    ...
    try {
      const { error } = await supabase.rpc('fn_registrar_movimiento_inventario', {
        ...
        p_origen: origen,
        p_cantidad: qty,
        p_costo_unitario: tipo === 'ENTRADA' && costoUnitario ? parseFloat(costoUnitario) : null,
        p_ref_tabla: null,
        p_ref_id: null,
        p_notas: notas || null,
      ...
      alertSuccess('Movimiento registrado', 'El movimiento se guardó correctamente')
      onSuccess()
      onClose()
    } catch (error) {
      console.error(error)
      alertError('Error al registrar', error.message || 'Ocurrió un error inesperado')
    } finally {
      setLoading(false)
    }
  }
  ...
  const newStock = isEntrada ? stockActual + (parseFloat(cantidad) || 0) : Math.max(0, stockActual - (parseFloat(cantidad) || 0))
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: '500px', backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
        ...
        <div style={{ padding: '24px', overflowY: 'auto', flex: 1 }}>
          {fetchingData ? (
            ...
            <form id="movimiento-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              
              {/* Selector de Producto o Info del Producto */}
              {!initialProductoId ? (
                <div>
              ...
              {productoId && almacenId && (
                <div style={{ 
                  ...
                  backgroundColor: isEntrada ? '#EAF3DE' : '#FCEBEB', 
                  border: `1px solid ${isEntrada ? '#C5E1A5' : '#F8BBD0'}`,
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ margin: 0, fontSize: '18px', fontWeight: 600, color: '#2C2C2A' }}>{stockActual}</p>
                  </div>
                  <ArrowRight size={20} color={isEntrada ? '#3B6D11' : '#A32D2D'} />
                  <div style={{ textAlign: 'center' }}>
                    <p style={{ margin: 0, fontSize: '11px', color: isEntrada ? '#3B6D11' : '#A32D2D', textTransform: 'uppercase' }}>Stock nuevo</p>
                    <p style={{ margin: 0, fontSize: '18px', fontWeight: 600, color: isEntrada ? '#3B6D11' : '#A32D2D' }}>{newStock}</p>
                  </div>
                </div>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#2C2C2A', marginBottom: '6px' }}>Cantidad *</label>
                  <input
                    type="number"
                    min="1"
                    step="0.01"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#2C2C2A', marginBottom: '6px' }}>Origen *</label>
                  <select
                    value={origen}
              ...
              {isEntrada && (
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#2C2C2A', marginBottom: '6px' }}>Costo Unitario (Opcional)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
              ...
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#2C2C2A', marginBottom: '6px' }}>Notas</label>
                <textarea
                  value={notas}
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid #E5E7EB', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
          ...
          <button
            ...
            style={{ 
              ...
              backgroundColor: isEntrada ? '#0F6E56' : '#A32D2D', display: 'flex', alignItems: 'center', gap: '8px',
