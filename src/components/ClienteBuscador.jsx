...
// Importaremos ClienteForm más tarde si el usuario lo requiere, por ahora omitido para crear clientes directo

export default function ClienteBuscador({ onSelect, error, placeholder = "Buscar cliente..." }) {
  ...
  async function buscarClientes() {
    setLoading(true);
    try {
      let query = supabase.from('clientes').select('*').eq('activo', true).is('deleted_at', null).order('nombre').limit(10);
      if (debouncedSearch) {
        query = query.or(`nombre.ilike.%${debouncedSearch}%,empresa.ilike.%${debouncedSearch}%`);
      }
              ...
              {options.map(cliente => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#E6F1FB', color: '#185FA5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '12px' }}>
                      {cliente.nombre?.substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <p style={{ margin: 0, fontSize: '13px', fontWeight: 500, color: '#2C2C2A' }}>{cliente.nombre}</p>
                      <p style={{ margin: 0, fontSize: '11px', color: '#888780' }}>{cliente.empresa || 'Particular'} • {cliente.ciudad || 'Sin ciudad'}</p>
                    </div>
                  </div>
